...
> **当前状态：active incubation baseline v0。** 目前已有 `1` 个 promoted Skill。第一次 canonical ownership transfer 完成后必须复盘一次，再决定是否升级为 v1。
...
可移植源码位于 [`skills/doc-steward`](skills/doc-steward)。它已经通过 clean-copy 结构与 CLI smoke test，但这不能证明某个具体 Agent runtime 会发现该副本。在对应 runtime 完成精确的 post-release discovery smoke test 前，本仓库不会把任何安装命令标成 verified。当前状态见 [Compatibility](docs/compatibility.md)。
